...
export default async function MissionsPage() {
  ...
  const { nextLaunch, recent, allLaunches } = data
  ...
  const next = nextLaunch || recent[0]
  const recentLaunches = recent.slice(0, 10)

  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-zinc-950 to-black text-white">

      {/* 1. LAUNCH COUNTER SQUARES – NOW 100% LIVE */}
      ...
      {/* 2. NEXT LAUNCH HERO – NOW REAL DATA */}
      {next && (
        <section className="relative py-32 px-6 overflow-hidden">
          ...
          <div className="relative max-w-5xl mx-auto text-center">
            ...
            <a href="https://youtube.com/spacex" target="_blank" rel="noopener noreferrer"
      ...
      {/* 3. RECENT LAUNCHES – NOW FROM REAL API */}
          ...
          {recentLaunches.map((l) => (
            <div key={l.id} className="group bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8 hover:border-red-800/50 transition-all">
              ...
              <p className="text-gray-400 mb-3">
                {new Date(l.date_utc).toLocaleDateString('en-US', { 
                  month: 'short', 
                  day: 'numeric',
                  hour: '2-digit',
                  minute: '2-digit'
              ...
              <div className="flex justify-between">
                <span className="bg-zinc-800 px-4 py-1 rounded-full text-sm">
                  {l.rocket.name || (l.rocket.id === '5e9d0d96eda699382d09d1ee' ? 'Starship' : 'Falcon')}
                </span>
                <span className={`text-sm ${l.success === false ? 'text-red-400' : 'text-green-400'}`}>
                  {l.success === null ? 'In Flight' : l.success ? 'Success' : 'Failure'}
                </span>
              </div>
            </div>
          ))}
      ...
      {/* 4. MILESTONES – UNCHANGED (they're historic) */}
